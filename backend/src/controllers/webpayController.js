...
exports.create = asyncHandler(async function (request, response, next) {
  try {
    ...
    let definitiveOrderId;
    try {
      if (cartId) {
        definitiveOrderId = await createOrder(cartId, newTotal);
        console.log("Order created with ID: ", definitiveOrderId);
      }
      if (!definitiveOrderId) {
        definitiveOrderId = buyOrder;
      }
    } catch (error) {
      return response.status(400).json({ error: "creacion de order fallida" });
    }
    ...
    const resetCart = async (cartId) => {
      await pool.query(`UPDATE cart_items SET active = 0 WHERE cart_id = $1`, [
        cartId,
      ]);
    };
